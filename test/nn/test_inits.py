...
from paddle_geometric.nn.inits import (
    glorot,
    glorot_orthogonal,
    ones,
    reset,
...
@pytest.mark.skip(reason="Missing 'reset_parameters' method in PaddlePaddle")
def test_reset():
    nn = Lin(16, 16)
    w = nn.weight.clone()
    reset(nn)

    assert not paddle.allclose(nn.weight, w)

    nn = Seq(Lin(16, 16), ReLU(), Lin(16, 16))
    w_1, w_2 = nn[0].weight.clone(), nn[2].weight.clone()
    reset(nn)
    assert not paddle.allclose(nn[0].weight, w_1)
    assert not paddle.allclose(nn[2].weight, w_2)
